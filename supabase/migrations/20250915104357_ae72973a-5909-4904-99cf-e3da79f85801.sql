-- Désactiver RLS sur toutes les tables pour permettre les modifications
...
DROP POLICY IF EXISTS "Users can view archives of their guild" ON public.archives;
DROP POLICY IF EXISTS "Users can insert archives for their guild" ON public.archives;
DROP POLICY IF EXISTS "Users can update archives for their guild" ON public.archives;

-- Supprimer toutes les contraintes de clés étrangères liées à guild_id
ALTER TABLE public.enterprises DROP CONSTRAINT IF EXISTS enterprises_guild_id_fkey;

-- Vider les tables pour permettre la modification des types
...
TRUNCATE public.archives CASCADE;

-- Modifier les types de colonnes guild_id de UUID vers TEXT
...
ALTER TABLE public.archives ALTER COLUMN guild_id TYPE TEXT;
...
-- Créer des politiques RLS très permissives pour les tests
CREATE POLICY "Allow all operations for authenticated users" ON public.guilds
FOR ALL TO authenticated 
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.enterprises
FOR ALL TO authenticated 
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.tax_brackets
FOR ALL TO authenticated 
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.wealth_tax_brackets
FOR ALL TO authenticated 
USING (true)
WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.archives
FOR ALL TO authenticated 
USING (true)
WITH CHECK (true);